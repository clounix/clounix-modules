...
#if defined(CLX_EN_LIGHTNING) && defined(CLX_EN_DAWN) && defined(CLX_EN_NAMCHABARWA)
#define NETIF_KNL_SUPPORT_CHIP          "Lightning/Dawn/Namchabarwa"
#elif defined(CLX_EN_LIGHTNING)
#define NETIF_KNL_SUPPORT_CHIP          "Lightning"
#elif defined(CLX_EN_NAMCHABARWA)
#define NETIF_KNL_SUPPORT_CHIP          "Namchabarwa"
#else
...
#else
        OSAL_PRINT(OSAL_DBG_COMMON,
                      "namchabarwa detected, but ops not support\n");
#endif
    }
    else
    {
...
#if defined (CLX_EN_NETIF)
    rc = _netif_knl_initDevOps(device_id,
                            &_osal_mdc_cb.dev[_osal_mdc_cb.dev_num]._netif_knl_cb.ops);
    if(CLX_E_OK != rc)
    {
...
static inline CLX_ERROR_NO_T
_osal_mdc_waitEvent(
    AML_DEV_MSI_DATA_T              *ptr_intr_data)
{
    unsigned long       flags = 0;

    if(0 != wait_event_interruptible(_osal_mdc_isr_wait, _osal_mdc_isr_rd_idx != _osal_mdc_isr_wr_idx))
    {
        return CLX_E_OTHERS;
    }
    /* get irq data */
    ...
    void                *ptr_cookie)
{
    ...
    else
    {
        if (NETIF_KNL_DEVICE_IS_NAMCHABARWA(ptr_dev->ptr_pci_dev->device))
        {
            osal_mdc_writePciReg(ptr_dev->unit, HAL_MT_NAMCHABARWA_PKT_PDMA_CFG_PDMA2PCIE_INTR_MASK_ALL,
        ...
        //     osal_mdc_writePciReg(ptr_dev->unit, HAL_MT_NAMCHABARWA_PKT_PDMA_CFG_PDMA2PCIE_INTR_MASK_ALL,
        //         &mask_val, sizeof(UI32_T));
        // osal_mdc_writePciReg(ptr_dev->unit, HAL_MT_NAMCHABARWA_INTR_TOP_MASK_REG, &mask_val, sizeof(UI32_T));
        }
#if defined(CLX_LINUX_USER_MODE)
    /* Notify user process */
    ...
    AML_DEV_ISR_DATA_T  *ptr_cookie)
{
    ...
    if (NULL == ptr_dev->isr_callback)
    {
        ...
        if (intr_mode == OSAL_INTR_MODE_MSI)
        {
            ...
            {
                linux_rc = request_irq(ptr_dev->irq + i, _osal_mdc_systemIntrCallback,
    ...
    void            *ptr_data)
{
    ...
    if (ptr_cb->dev_num >= OSAL_MDC_MAX_CHIPS_PER_SYSTEM)
    {
        OSAL_PRINT(OSAL_DBG_ERR,"dev num=%d > max support num=%d\n",
                     ptr_ioctl_data.dev_num, OSAL_MDC_MAX_CHIPS_PER_SYSTEM);
    }
