...
static HAL_LT_DAWN_PKT_NETIF_PROFILE_T              *_ptr_hal_lt_dawn_pkt_profile_entry[HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX] = {0};
...
#define HAL_LT_DAWN_PKT_GET_PORT_PROFILE_LIST(port)         (_hal_lt_dawn_pkt_port_db[port].ptr_profile_list)
...
/* FUNCTION NAME: hal_lt_dawn_pkt_setPortAttr
 * PURPOSE:
...
static void
_hal_lt_dawn_pkt_lockRxChannelAll(
    const UI32_T                        unit)
{
    UI32_T                              rch;
...
static void
_hal_lt_dawn_pkt_unlockRxChannelAll(
    const UI32_T                        unit)
{
    UI32_T                              rch;
    HAL_LT_DAWN_PKT_RX_PDMA_T               *ptr_rx_pdma;
    ...
    BOOL_T                          *ptr_hit_prof)
{
    ...
    if ((ptr_profile->flags & (HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_PATTERN_0 |
    ...
    void                            *ptr_data)
{
    ...
    if ((que_cnt > 0) && (queue < HAL_LT_DAWN_PKT_RX_QUEUE_NUM))
    {
        ...
        if (CLX_E_OK == rc)
        {
            ...
            osal_io_copyFromUser(&ioctl_data, ptr_cookie, sizeof(HAL_LT_DAWN_PKT_IOCTL_RX_COOKIE_T));
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_resumeAllIntf(
    const UI32_T                        unit)
{
    struct net_device                   *ptr_net_dev = NULL;
    UI32_T                              port;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_suspendAllIntf(
    const UI32_T                        unit)
{
    struct net_device                   *ptr_net_dev = NULL;
    UI32_T                              port;
...
/* FUNCTION NAME: hal_lt_dawn_pkt_setRxKnlConfig
 * PURPOSE:
...
/* FUNCTION NAME: hal_lt_dawn_pkt_getRxKnlConfig
 * PURPOSE:
...
/* FUNCTION NAME: hal_lt_dawn_pkt_deinitPktDrv
 * PURPOSE:
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_addProfToList(
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T         *ptr_new_profile,
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_addProfToAllIntf(
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T         *ptr_new_profile)
{
    UI32_T                              port;
...
_hal_lt_dawn_pkt_delProfFromListById(
    const UI32_T                            id,
    HAL_LT_DAWN_PKT_PROFILE_NODE_T              **pptr_profile_list)
{
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_delProfFromAllIntfById(
    const UI32_T                        id)
{
    UI32_T                              port;
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T         *ptr_port_db;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_allocProfEntry(
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T         *ptr_profile)
{
    UI32_T          idx;
...
static HAL_LT_DAWN_PKT_NETIF_PROFILE_T  *
_hal_lt_dawn_pkt_freeProfEntry(
    const UI32_T                 id)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T         *ptr_profile = NULL;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyAllIntf(
    const UI32_T                        unit)
{
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T         *ptr_port_db;
    UI32_T                              port = 0;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_delProfListOnAllIntf(
    const UI32_T                        unit)
{
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T         *ptr_port_db;
    UI32_T                              port = 0;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyAllProfile(
    const UI32_T                        unit)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T         *ptr_profile;
    UI32_T                              prof_id;
...
/* FUNCTION NAME: hal_lt_dawn_pkt_initPktDrv
 * PURPOSE:
...
static int
_hal_lt_dawn_pkt_net_dev_ioctl(
    struct net_device           *ptr_net_dev,
...
static void
_hal_lt_dawn_pkt_net_dev_tx_timeout(
    struct net_device           *ptr_net_dev,
...
_hal_lt_dawn_pkt_net_dev_get_stats(
    struct net_device           *ptr_net_dev)
{
    struct net_device_priv      *ptr_priv = netdev_priv(ptr_net_dev);
...
static int
_hal_lt_dawn_pkt_net_dev_set_mtu(
    struct net_device           *ptr_net_dev,
    int                         new_mtu)
{
...
static int
_hal_lt_dawn_pkt_net_dev_set_mac(
    struct net_device           *ptr_net_dev,
    void                        *ptr_mac_addr)
{
...
static void
_hal_lt_dawn_pkt_net_dev_set_rx_mode(
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,10,0)
static int
_hal_lt_dawn_pkt_net_dev_ethtool_get(
    struct net_device           *ptr_dev,
    ...
    void                                *ptr_data)
{
    ...
    if (ptr_port_db->ptr_net_dev == NULL)
    {
        ...
        ptr_net_dev = alloc_netdev(sizeof(struct net_device_priv),
                                   net_intf.name, NET_NAME_UNKNOWN, _hal_lt_dawn_pkt_setup);
#else
        ptr_net_dev = alloc_netdev(sizeof(struct net_device_priv),
                                   net_intf.name, _hal_lt_dawn_pkt_setup);
#endif
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_traverseProfList(
    UI32_T                          intf_id,
    HAL_LT_DAWN_PKT_PROFILE_NODE_T      *ptr_prof_list)
{
    ...
    for (port = 0; port < HAL_LT_DAWN_PKT_MAX_PORT_NUM; port++)
    {
        ...
        if (NULL != ptr_port_db->ptr_net_dev)       /* valid intf */
        {
            if (ptr_port_db->meta.id == net_intf.id)
            {
                ...
                osal_io_copyToUser(&ptr_cookie->net_intf, &ptr_port_db->meta, sizeof(HAL_LT_DAWN_PKT_NETIF_INTF_T));
                rc = CLX_E_OK;
                break;
            }
        }
    ...
    void                                *ptr_data)
{
    ...
    if (CLX_E_OK == rc)
    {
        ...
        osal_io_copyToUser(&ptr_cookie->net_profile, ptr_profile, sizeof(HAL_LT_DAWN_PKT_NETIF_PROFILE_T));
    }
    else
    {
        OSAL_PRINT((OSAL_DBG_PROFILE | OSAL_DBG_ERR),
    ...
    void                                *ptr_data)
{
    ...
    osal_io_copyFromUser(&profile, &ptr_cookie->net_profile, sizeof(HAL_LT_DAWN_PKT_NETIF_PROFILE_T));
