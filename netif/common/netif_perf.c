    ...
    void                        *ptr_sw_gpd)
{
    ...
    {
        ret = hal_lt_lightning_pkt_prepareGpd(unit,phy_addr,len,port,(HAL_LT_LIGHTNING_PKT_TX_SW_GPD_T*)ptr_sw_gpd);
    }
    else if (NETIF_KNL_DEVICE_IS_NAMCHABARWA(dev_id))
    {
        ptr_cookie = ((HAL_MT_NAMCHABARWA_PKT_TX_SW_GPD_T*)ptr_sw_gpd)->ptr_cookie;
        ret = hal_mt_namchabarwa_pkt_prepareGpd(unit,phy_addr, len, port,
                (HAL_MT_NAMCHABARWA_PKT_TX_SW_GPD_T*)ptr_sw_gpd, 
                (HAL_MT_NAMCHABARWA_PKT_PPH_L2_T *)((void *)ptr_cookie + HAL_MT_NAMCHABARWA_PKT_EMAC_SZ));
    }
    else
    {
        OSAL_PRINT(OSAL_DBG_ERR, "unknown chip family, dev_id=0x%x\n",dev_id);
    ...
    void                            *ptr_sw_gpd)
{
    ...
    {
        ret = hal_mt_namchabarwa_pkt_sendGpd(unit,channel,(HAL_MT_NAMCHABARWA_PKT_TX_SW_GPD_T*)ptr_sw_gpd);
    }
    else
    {
        OSAL_PRINT(OSAL_DBG_ERR, "unknown chip family, dev_id=0x%x\n",dev_id);
    ...
    CLX_TIME_T                  duration)
{
    ...
    count += sprintf(buf + count, "------------------------------------\n");
    count += sprintf(buf + count, "avg. packet rate (pps)  : %llu\n", (num * 1000) / (duration / 1000));
        ...
        while (_perf_tx_perf_cb.send_ok[channel] < num)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        OSAL_PRINT(OSAL_DBG_ERR, "***Error***, Tch-%d send fail over %d packet(s). (rc: %d)\n",
    ...
    const UI32_T                len)
{
    ...
    else if ((_perf_rx_perf_cb.recv_pass % _perf_rx_perf_cb.target_num) == _perf_rx_perf_cb.target_num - 1)
    {
...
static ssize_t perf_tx_test(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    return _perf_showPerf(buf, PERF_DIR_TX,
        _perf_tx_perf_cb.tx_channel_bitmap, _perf_tx_perf_cb.target_len, tx_pkt_cnt, tx_end_intr - tx_start_intr, end_time - start_time);
}

static ssize_t perf_rx_test(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    CLX_TIME_T                  start_time;
    CLX_TIME_T                  end_time;
    UI32_T                      unit = 0;
    ...
    return _perf_showPerf(buf, PERF_DIR_RX,
        _perf_rx_perf_cb.rx_channel, _perf_rx_perf_cb.target_len, _perf_rx_perf_cb.target_num, rx_end_intr - rx_start_intr, end_time - start_time);
}

static ssize_t get_rx_target_num(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", _perf_rx_perf_cb.target_num);
}

static ssize_t set_rx_target_num(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret;
...
static ssize_t get_rx_target_len(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", _perf_rx_perf_cb.target_len);
}

static ssize_t set_rx_target_len(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret;
...
static ssize_t get_tx_target_num(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", _perf_tx_perf_cb.target_num);
}

static ssize_t set_tx_target_num(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret;
...
static ssize_t get_tx_target_len(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", _perf_tx_perf_cb.target_len);
}

static ssize_t set_tx_target_len(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret;
...
static ssize_t get_tx_channel_bmp(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", _perf_tx_perf_cb.tx_channel_bitmap);
}

static ssize_t set_tx_channel_bmp(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int ret;
...
static ssize_t perf_get_help(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    char *help = 
...
static struct kobj_attribute perf_test_help_attribute =
    __ATTR(help, 0660, perf_get_help, NULL);

static struct kobj_attribute perf_rx_test_attribute =
    __ATTR(rx_test, 0660, perf_rx_test, NULL);

static struct kobj_attribute perf_tx_test_attribute =
    __ATTR(tx_test, 0660, perf_tx_test, NULL);
