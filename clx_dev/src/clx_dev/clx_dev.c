    ...
    const UI32_T        len)
{
    ...
    OSAL_CHECK_PTR(ptr_base_addr);

    if (len % OSAL_MDC_PCI_BUS_WIDTH)
    {
        OSAL_PRINT(OSAL_DBG_ERR, "Must be 4-byte alignment, %d\n", len);
    ...
    const UI32_T        unit)
{
    ...
    {
        OSAL_PRINT(OSAL_DBG_ERR,"please connect isr first.");
    ...
    const UI32_T        unit)
{
    ...
    switch (_osal_mdc_ioctl_dev[unit].id.device & 0xFF00)
    {
        ...
        case HAL_DEVICE_ID_CL8300:
            ...
            break;    
        default:
            OSAL_PRINT(OSAL_DBG_ERR,"Wrong device id:0x%x\n",_osal_mdc_ioctl_dev[unit].id.device);
            break;
    }
    return device;
}
    ...
    void            *ptr_data)
{
    ...
    _osal_mdc_getPciInfoToIoctlData(ptr_dev_list, ptr_ioctl_data);
    if(CLX_E_OK != rc)
    {
...
#define OSAL_MDC_IOCTL_LOCAL_BUF_SIZE           (128)
    ...
    if (NULL == ptr_cb->callback[type])
    {
        OSAL_PRINT(OSAL_DBG_ERR,"invalid ioctl, cmd=%u, arg=%lu, type=%d\n", cmd, arg, type);
        return (linux_rc);
    }
