...
static CLX_SEMAPHORE_ID_T recv_lock;
int recv_pass = 0 ;
static UI32_T flag = 0;
//DEFINE_PER_CPU(int,recv_pass);
static void
_perf_rxInit(
    const UI32_T                unit,
    const UI32_T                rx_channel,
    const UI32_T                len)
{
    ...
    if(flag == 0)
    {
        osal_createIsrLock("recv_lock", &recv_lock);
        flag ++;
    }

    //printk("_perf_rxInit  recv_pass:%d\n",recv_pass);
    //int a = recv_pass;
    //osal_takeIsrLock(&recv_lock, &irq_flag);
    recv_pass = 0;
    //osal_giveIsrLock(&recv_lock,&irq_flag);
    printk("22_perf_rxInit  recv_pass:%d \n",recv_pass);
    ...
    const UI32_T                len)
{
    ...
    else if (recv_pass == _perf_rx_perf_cb.target_num)
    {
        printk("2222  %d\n",len);
        osal_triggerEvent(&_perf_rx_perf_cb.end_sync);
    }
    else if (1 == (recv_pass % PERF_RX_PERF_MESG))
    {
        printk("R  %d",recv_pass);
    }
    ...
    BOOL_T                      test_skb)
{
    ...
    else if (rx_channel > 0)
    {
        ...
        _perf_showPerf(PERF_DIR_RX,
            rx_channel, len, recv_pass, rx_end_intr - rx_start_intr, end_time - start_time);
    }
